        ...
        <ul className="space-y-2">
          ...
          <li className={`${isCollapsed ? 'flex justify-center' : ''}`}>
            <div className={`${isCollapsed ? 'w-10' : 'w-full '}`}>
              ...
              >
                <div className="flex items-center rounded-l-md hover:bg-purple-300 py-2 pl-2">
                  <FaPenToSquare className={`w-5 h-5 text-purple-900 min-w-[1.25rem] ${isCollapsed ? 'ml-0' : 'ml-4'} `}/>
                  {!isCollapsed && (
                    <span className=" border-r border-purple-500 pr-8 ml-2 font-semibold">Start writing</span>
                  )}
                </div>
                {!isCollapsed && (
