      ...
      <nav className="mt-4 flex-grow overflow-y-auto">
        <ul className="space-y-4">
          ...
          <li className={`${isCollapsed ? 'flex justify-center' : ''}`}>
            <div className={`${isCollapsed ? 'w-10' : 'mx-4 w-[calc(100%-2rem)]'}`}>
              <button 
                onClick={() => {
                  if (!isCollapsed) {
              ...
              >
                <FaPenToSquare className="w-5 h-5 text-purple-900 ml-2" />
                {!isCollapsed && (
                  <>
                    <span className="ml-2 flex-grow">Start writing</span>
                    {openSections.writing ? <IoIosArrowUp className="mr-2 text-purple-900" /> : <IoIosArrowDown className="mr-2 text-purple-900" />}
                  </>
                )}
              </button>
