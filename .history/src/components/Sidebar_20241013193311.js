    ...
    <div className={`bg-white dark:bg-gray-900 px-2 text-gray-600 dark:text-gray-300 h-screen flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
      ...
      <nav className="mt-4 flex-grow overflow-y-auto">
        <ul className="space-y-2">
          ...
          <li className={`${isCollapsed ? 'flex justify-center' : ''}`}>
            <div className={`${isCollapsed ? 'w-10' : 'w-full px-4'}`}>
              <button 
                ...
                className={`flex items-center bg-[#dfdaf8] py-2 mt-5 border border-purple-600 text-purple-900 rounded-lg ${isCollapsed ? 'w-10 h-10 justify-center' : 'w-full'}`}
              >
          ...
          {/* Posts Section */}
          <li>
            <Link to="/dashboard/posts" className={getLinkClasses('/dashboard/posts') + ' mt-5'}>
