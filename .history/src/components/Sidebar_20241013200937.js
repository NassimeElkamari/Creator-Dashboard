          ...
          <li className={`${isCollapsed ? 'flex justify-center' : ''}`}>
            <div className={`${isCollapsed ? 'w-10' : 'w-full '}`}>
              <button 
                ...
                className={`flex items-center bg-[#dfdaf8]  mt-3 border border-purple-600 text-purple-900 rounded-lg ${isCollapsed ? 'w-10 h-10 justify-center' : 'w-full '}`}
              >
                <div className="flex items-center rounded-l-md border-r border-purple-500 hover:bg-purple-300 py-2 pl-2">
                  <FaPenToSquare className={`w-5 h-5 text-purple-900 min-w-[1.25rem] ${isCollapsed ? 'ml-0' : 'ml-4'} `}/>
                  {!isCollapsed && (
                    <span className="  pr-8 ml-2 font-semibold">Start writing</span>
                  )}
                </div>
                {!isCollapsed && (
