...
const Sidebar = () => {
  ...
  const getLinkClasses = (path) => {
    ...
    return `flex items-center gap-4 p-4 text-lg ${isCollapsed ? 'justify-center' : ''} ${
      isActive
  ...
  return (
    <div className={`bg-white dark:bg-gray-900 px- text-gray-600 dark:text-gray-300 h-screen flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
      {/* Branding */}
      ...
      <nav className="mt-4 flex-grow overflow-y-auto">
        <ul className="space-y-2">
          ...
          <li className={`${isCollapsed ? 'flex justify-center' : ''}`}>
            <div className={`${isCollapsed ? 'w-10' : 'w-full px-4'}`}>
              <button 
                onClick={() => {
                  if (!isCollapsed) {
              ...
              >
                <FaPenToSquare className="w-5 h-5 text-purple-900 min-w-[1.25rem]" />
                {!isCollapsed && (
                  <>
                    <span className="ml-2 flex-grow">Start writing</span>
                    {openSections.writing ? <IoIosArrowUp className="ml-2 text-purple-900" /> : <IoIosArrowDown className="ml-2 text-purple-900" />}
                  </>
                )}
              </button>
