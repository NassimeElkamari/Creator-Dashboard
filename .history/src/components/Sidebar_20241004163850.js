...
const Sidebar = () => {
  ...
  const getLinkClasses = (path) => {
    return location.pathname === path
      ? 'ml-4 gap-2 p-4 text-lg text-blue-700 border-l-4 font-bold border-blue-700 flex items-center' // Updated active link style
      : 'ml-4 gap-2 p-4 text-lg  flex items-center'; // Default link style
  };

  return (
    <div className="w-60 h-screen bg-white text-gray-600">
      <nav className="mt-10">
