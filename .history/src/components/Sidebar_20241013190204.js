    ...
    <div className={`bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-300 h-screen flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'}`}>
      ...
      <nav className="mt-4 flex-grow overflow-y-auto">
        <ul className="space-y-4">
          ...
          {/* Start Writing */}
          <li>
            <div className={`ml-4 ${isCollapsed ? 'mx-auto w-10' : ''}`}>
              <button 
              ...
              >
                ...
                {!isCollapsed && (
                  <>
                    <span className="ml-4">Start writing</span>
                    {openSections.writing ? <IoIosArrowUp className="ml-auto mr-4 text-purple-900" /> : <IoIosArrowDown className="ml-auto mr-4 text-purple-900" />}
                  </>
                )}
              </button>
              {!isCollapsed && openSections.writing && (
                <ul className="mt-2 space-y-2 bg-white bg-opacity-150 dark:bg-purple-900 dark:bg-opacity-30 rounded-lg p-2">
                  <li>
                    <Link onClick={handleUploadVideo} className="block px-4 py-2 text-black hover:bg-gray-200 rounded">
                      Upload Video
                    </Link>
                  </li>
                  <li>
          ...
          {/* Audience Section */}
          <li>
            <button onClick={() => toggleSection('audience')} className={`flex items-center ml-4 p-4 w-full ${isCollapsed ? 'justify-center' : 'text-left gap-4'}`}>
            ...
            {!isCollapsed && openSections.audience && (
          ...
          {/* Analyze Section */}
          <li>
            <button onClick={() => toggleSection('analyze')} className={`flex items-center ml-4 p-4 w-full ${isCollapsed ? 'justify-center' : 'text-left gap-4'}`}>
            ...
            {!isCollapsed && openSections.analyze && (
              <ul className="absolute top-full left-0 w-full pl-12 space-y-2 bg-white dark:bg-gray-900 z-10">
                <li>
