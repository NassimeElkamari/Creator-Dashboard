          ...
          <li className={`${isCollapsed ? 'flex justify-center' : ''}`}>
            <div className={`${isCollapsed ? 'w-10' : 'w-full '}`}>
              <button 
                ...
                className={`flex items-center justify-between bg-[#dfdaf8] py-2 mt-5 border border-purple-600 text-purple-900 rounded-lg ${isCollapsed ? 'w-10 h-10 justify-center' : 'w-full pl-2'}`}
              >
                <div className="flex items-center">
                  <FaPenToSquare className={`w-5 h-5 text-purple-900 min-w-[1.25rem] ${isCollapsed ? 'ml-0' : 'ml-4'} `}/>
                  {!isCollapsed && (
                    <span className="flex-grow border-r border-purple-500 pr-2 ml-[-17%] font-semibold">Start writing</span>
                  )}
                </div>
                {!isCollapsed && (
