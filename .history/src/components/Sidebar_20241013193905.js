          ...
          <li className={`${isCollapsed ? 'flex justify-center' : ''}`}>
            <div className={`${isCollapsed ? 'w-10' : 'w-full '}`}>
              <button 
                ...
                className={`flex items-center bg-[#dfdaf8] py-2 pl-2 mt-5 border border-purple-600 text-purple-900 rounded-lg ${isCollapsed ? 'w-10 h-10 justify-center' : 'w-full'}`}
              >
                <FaPenToSquare className={`w-5 h-5 text-purple-900 min-w-[1.25rem] ml-4 ${isCollapsed ? 'ml-0' : ''}          `}/>
                {!isCollapsed && (
                  <>
